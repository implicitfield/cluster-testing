...
cat wg0-primary.conf | \
  perl -pe 's/PRIVATE_KEY/$ENV{PRIMARY_PRIVATE_KEY}/' | \
  perl -pe 's/INSTANCE_0_PUBLIC_KEY/$ENV{INSTANCE_0_PUBLIC_KEY}/' | \
  perl -pe 's/INSTANCE_1_PUBLIC_KEY/$ENV{INSTANCE_1_PUBLIC_KEY}/' | \
